...
public class OrderRepository implements ICrud<Order>  {
    ...
    public ArrayList<OrderDisplay> displayAll() {
        ...
        while(order.next()) {
            ...
            System.out.println("order test: "+order.getInt("id")+" "+fullname+" "+customerList.get(order.getInt("customer")).getName()+" "+modelList.get(order.getInt("model")).getName()+" "+ order.getDate("placement_date")+" "+order.getDate("delivery_date")+" "+statusList.get(order.getInt("status")));
            o++;
        }
